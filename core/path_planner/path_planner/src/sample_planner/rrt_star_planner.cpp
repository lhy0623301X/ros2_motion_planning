 ...
 */
RRTStarPathPlanner::RRTStarPathPlanner(costmap_2d::Costmap2DROS* costmap_ros)
 ...
 */
bool RRTStarPathPlanner::plan(const Point3d& start, const Point3d& goal, Points3d* path,
                              Points3d* expand) {
 ...
 * @return nearest node
 */
RRTStarPathPlanner::Node
RRTStarPathPlanner::_findNearestPoint(std::unordered_map<int, Node>& list, Node& node) {
  ...
  if (!isCollision(new_node, nearest_node)) {
    ...
    for (auto& p : sample_list_) {
      ...
      double new_dist =
          std::hypot(p.second.x() - new_node.x(), p.second.y() - new_node.y());
      if (new_dist < config_.sample_planner().optimization_radius()) {
