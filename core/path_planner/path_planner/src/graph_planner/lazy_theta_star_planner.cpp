 ...
 */
bool LazyThetaStarPathPlanner::plan(const Point3d& start, const Point3d& goal,
  ...
  while (!open_list.empty()) {
    ...
    if (current == goal_node) {
      const auto& backtrace =
          _convertClosedListToPath<Node>(closed_list_, start_node, goal_node);
  ...
  if (isCollision(parent, node)) {
    ...
    for (const auto& m : motions) {
      ...
      if (find_parent_new != closed_list_.end()) {
        ...
        const double dist =
            std::hypot(parent_new.x() - node.x(), parent_new.y() - node.y());
