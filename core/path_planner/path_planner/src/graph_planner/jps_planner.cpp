 ...
 */
JPSPathPlanner::JPSPathPlanner(costmap_2d::Costmap2DROS* costmap_ros)
 ...
 */
bool JPSPathPlanner::plan(const Point3d& start, const Point3d& goal, Points3d* path,
                          Points3d* expand) {
  ...
  // initialization
  for (int i = 4; i < 6; i++) {  // explore left-top and right-bottom from current
    _checkSlashLine(dirs_[i], start_, open_list_);
  }
  for (int i = 6; i < 8; i++) {  // explore left-bottom and right-top from next
  ...
  while (!open_list_.empty()) {
    ...
    if (current == goal_) {
      ...
      const auto& backtrace =
          _convertClosedListToPath<JNode>(closed_list_, start_, goal_);
...
bool JPSPathPlanner::_forceNeighborDetect(int dir, int cur_id, std::vector<int>& fn_id) {
  fn_id.clear();
  std::array<int, 2> delta_obs = { dirs_[dir_to_obs_id_.at(dir).first],
                                   dirs_[dir_to_obs_id_.at(dir).second] };

  // horizon or vertical
  if (dir == 1 || dir == -1 || dir == nx_ || dir == -nx_) {
    for (int i = 0; i < 2; i++) {
      if (costmap_->getCharMap()[cur_id + delta_obs[i]] >=
              costmap_2d::LETHAL_OBSTACLE * config_.obstacle_inflation_factor() &&
          costmap_->getCharMap()[cur_id + delta_obs[i] + dir] <
              costmap_2d::LETHAL_OBSTACLE * config_.obstacle_inflation_factor()) {
  ...
  // slash
    ...
    for (int i = 0; i < 2; i++) {
      if (costmap_->getCharMap()[cur_id + delta_obs[i]] >=
              costmap_2d::LETHAL_OBSTACLE * config_.obstacle_inflation_factor() &&
          costmap_->getCharMap()[cur_id + 2 * delta_obs[i] + dir] <
              costmap_2d::LETHAL_OBSTACLE * config_.obstacle_inflation_factor()) {
...
bool JPSPathPlanner::_checkStraightLine(int dir, const JNode& node, OpenList& open_list) {
  ...
  while (true) {
    ...
    // goal is jump point
    if (pt == goal_.id()) {
      open_list.emplace(goal_.x(), goal_.y(),
    ...
    if (_forceNeighborDetect(dir, pt, force_neighbors)) {
      for (int fn_id : force_neighbors) {
        ...
        if (fn_id < 0 || fn_id >= map_size_ ||
            closed_list_.find(fn_id) != closed_list_.end()) {
          continue;
        }
        open_list.emplace(
            pt_x, pt_y, node.g() + std::hypot(node.x() - pt_x, node.y() - pt_y),
 ...
 * direction
 */
bool JPSPathPlanner::_checkSlashLine(int dir, const JNode& node, OpenList& open_list,
                                     bool from_cur) {
  ...
  if (from_cur) {
    ...
    if (_checkStraightLine(x_dir, node, open_list) ||
        _checkStraightLine(y_dir, node, open_list)) {
      find_jp = true;
    }
  }
  ...
  while (true) {
    ...
    if (_checkStraightLine(x_dir, temp, open_list) ||
        _checkStraightLine(y_dir, temp, open_list)) {
      if (!(temp.id() < 0 || temp.id() >= map_size_ ||
            closed_list_.find(temp.id()) != closed_list_.end())) {
