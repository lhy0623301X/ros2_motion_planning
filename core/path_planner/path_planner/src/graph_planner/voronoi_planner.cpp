 ...
 */
VoronoiPathPlanner::VoronoiPathPlanner(costmap_2d::Costmap2DROS* costmap_ros,
 ...
 */
bool VoronoiPathPlanner::plan(const Point3d& start, const Point3d& goal, Points3d* path,
                              Points3d* expand) {
  ...
  for (int j = 0; j < ny_; j++) {
    for (int i = 0; i < nx_; i++) {
      voronoi_diagram_[i][j].dist =
          voronoi_.getDistance(i, j) * costmap_->getResolution();
 ...
 * @return  true if path found, else false
 */
bool VoronoiPathPlanner::searchPathWithVoronoi(const Node& start, const Node& goal,
                                               std::vector<Node>& path, Node* v_goal) {
  path.clear();
  ...
  while (!open_list.empty()) {
    ...
    // goal found
    if ((current == goal) ||
        (v_goal == nullptr ? false :
                             voronoi_diagram_[static_cast<unsigned int>(current.x())]
                                             [static_cast<unsigned int>(current.y())]
                                                 .is_voronoi)) {
    ...
    for (const auto& m : motions) {
      ...
      if ((node_new.id() < 0) || (node_new.id() >= map_size_) ||
          (voronoi_diagram_[static_cast<unsigned int>(node_new.x())]
                           [static_cast<unsigned int>(node_new.y())]
                               .dist < circumscribed_radius_))
        continue;
