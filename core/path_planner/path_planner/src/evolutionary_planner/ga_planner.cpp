 ...
 */
GAPathPlanner::GAPathPlanner(costmap_2d::Costmap2DROS* costmap_ros)
  : PathPlanner(costmap_ros), bspline_gen_(std::make_unique<BSplineCurve>()) {
  ga_config_ = config_.evolutionary_planner().ga_planner();
  inherited_genets_.emplace_back(Points2d(ga_config_.control_point_num()), 0.0);
}
 ...
 */
bool GAPathPlanner::plan(const Point3d& start, const Point3d& goal, Points3d* path,
                         Points3d* expand) {
  ...
  if ((ga_config_.population_num() <= 0) || (ga_config_.population_num() % 2 != 0)) {
    R_ERROR << " GA : The parameter n_genets is set improperly. Please ensure that it is "
  ...
  for (int i = 0; i < ga_config_.population_num(); ++i) {
    Points2d init_position;

    if ((i < ga_config_.inherited_num()) &&
        (inherited_genets_.size() == ga_config_.inherited_num())) {
  ...
  for (size_t iter = 0; iter < ga_config_.max_iterations(); iter++) {
    selection(genets_swarm, genets_parent);

    genets_children = genets_parent;
    std::rotate(genets_children.begin(), genets_children.begin() + 1,
                genets_children.end());

    std::vector<std::thread> genets_list = std::vector<std::thread>(genets_parent.size());
    for (size_t i = 0; i < genets_parent.size(); ++i)
      genets_list[i] =
          std::thread(&GAPathPlanner::optimizeGenets, this, std::cref(genets_parent[i]),
                      std::ref(genets_children[i]), std::ref(best_genet), i,
    ...
    std::copy(genets_parent.begin(), genets_parent.end(),
              genets_swarm.begin() + genets_children.size());
  }
  ...
  std::sort(
      genets_swarm.begin(), genets_swarm.end(),
      [](const Genets& a, const Genets& b) { return a.best_fitness > b.best_fitness; });
  inherited_genets_.clear();

  for (size_t inherit = 0; inherit < ga_config_.inherited_num(); ++inherit)
 ...
 * @param goal      goal node
 */
void GAPathPlanner::initializePositions(PositionSequence& initial_positions,
                                        const Point3d& start, const Point3d& goal) {
  // Use a random device and engine to generate random numbers
  std::random_device rd;
  std::mt19937 gen(rd());
  int x[ga_config_.control_point_num()], y[ga_config_.control_point_num()];
  int point_id, pos_id;
  ...
  for (int i = 0; i < ga_config_.population_num(); ++i) {
    ...
    while (point_id < ga_config_.control_point_num()) {
      if (config_.evolutionary_planner().init_mode() ==
    ...
    // sort
    if (x_order) {
      std::sort(x, x + ga_config_.control_point_num(),
    ...
    if (y_order) {
      std::sort(y, y + ga_config_.control_point_num(),
                [](int a, int b) { return a < b; });
    } else {
      std::sort(y, y + ga_config_.control_point_num(),
                [](int a, int b) { return a > b; });
    ...
    for (int j = 0; j < ga_config_.control_point_num(); ++j) {
 ...
 */
void GAPathPlanner::selection(const std::vector<Genets>& population,
                              std::vector<Genets>& selected_population) {
  // Calculate selection mode
  int select_mode =
      (static_cast<int>(100 * ga_config_.prob_selection()) % 2 == 0) ? 0 : 1;
  // Calculate the number of individuals to be selected
  size_t select_num =
      static_cast<size_t>(population.size() * ga_config_.prob_selection());
  ...
  if (select_mode) {
    ...
    for (size_t i = 1; i < select_num; ++i) {
      ...
      auto it = std::find_if(acc_p.begin(), acc_p.end(),
                             [rand_num](double val) { return val > rand_num; });

      if (it != acc_p.end()) {
        // Get the index of the first cumulative probability greater than the random
        // number
 ...
 */
void GAPathPlanner::optimizeGenets(const Genets& genets_p, Genets& genets_c,
                                   Genets& best_genet, const int& i, std::mt19937& gen,
                                   Points3d* expand) {
  std::uniform_real_distribution<double> dist_d(0.0, 1.0);
  std::uniform_int_distribution<int> dist_i(0, ga_config_.control_point_num() - 1);
  double random1 = dist_d(gen);
  double random2 = dist_d(gen);

  // Perform crossover operation
  if (random1 < ga_config_.prob_crossover()) {
    int random_id = dist_i(gen);
    std::copy(genets_p.position.begin() + random_id, genets_p.position.end(),
              genets_c.position.begin() + random_id);
  }

  // Perform mutation operation
  if (random2 < ga_config_.prob_mutation()) {
    int random_id = dist_i(gen);

    // If it exceeds a certain number of times, it is considered that the mutation has
    // failed
    for (size_t i = 0; i < 10; i++) {
      std::uniform_real_distribution<double> dist_m(-1.0, 1.0);
      double random3 = dist_m(gen);
      double random4 = dist_m(gen);
      double x = genets_c.position[random_id].x() + random3 * ga_config_.max_speed();
      double y = genets_c.position[random_id].y() + random4 * ga_config_.max_speed();
