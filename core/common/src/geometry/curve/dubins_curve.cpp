...
bool DubinsCurve::generation(const Point3d& start, const Point3d& goal, Points3d& path)
{
  ...
  for (int j = 0; j < 3; j++)
  {
    ...
    // current path length
    double l = d_l;
    while (std::fabs(l) <= std::fabs(seg_length))
    {
      i += 1;
      const auto& inter_pose = interpolate(m, l, { x, y, yaw });
      path_x[i] = inter_pose.x(), path_y[i] = inter_pose.y(), path_yaw[i] = inter_pose.theta();
      l += d_l;
    }
    i += 1;
    const auto& inter_pose = interpolate(m, seg_length, { x, y, yaw });
    path_x[i] = inter_pose.x(), path_y[i] = inter_pose.y(), path_yaw[i] = inter_pose.theta();
  }
  ...
  for (size_t i = 0; i < path_x.size(); i++)
  {
    path.emplace_back(P(0, i) + sx, P(1, i) + sy, rmp::common::math::pi2pi(path_yaw[i] + theta));
  }
