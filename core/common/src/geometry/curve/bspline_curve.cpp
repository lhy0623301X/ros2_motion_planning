 ...
 */
double BSplineCurve::baseFunction(int i, int k, double t, const std::vector<double>& knot)
{
  double Nik_t = 0;
 ...
 */
Points2d BSplineCurve::bsplineGeneration(int k, const std::vector<double>& knot, Points2d control_pts)
{
 ...
 */
bool BSplineCurve::generation(const Point3d& start, const Point3d& goal, Points3d& path)
{
 ...
 * @return true if generate successfully, else failed
 */
bool BSplineCurve::run(const Points2d& points, Points3d& path)
{
  if (points.size() < 4)
  {
    return false;
  ...
  else
  {
    ...
    const auto& waypoints = bsplineGeneration(order_, knot, control_pts);
    if (waypoints.size() < 1)
    {
      return false;
    }
    ...
    path.emplace_back(waypoints[0].x(), waypoints[0].y(), start_angle_);
    for (size_t i = 1; i < waypoints.size() - 1; i++)
    {
      auto prev_interp_vec = Vec2d(waypoints[i - 1].x(), waypoints[i - 1].y());
      auto curr_interp_vec = Vec2d(waypoints[i].x(), waypoints[i].y());
      auto next_interp_vec = Vec2d(waypoints[i + 1].x(), waypoints[i + 1].y());
      ...
      path.emplace_back(waypoints[i].x(), waypoints[i].y(), dir.angle());
    }
    path.emplace_back(waypoints.back().x(), waypoints.back().y(), goal_angle_);
 ...
 * @return true if generate successfully, else failed
 */
bool BSplineCurve::run(const Points3d& points, Points3d& path)
{
